...
window.startMostPopular = async function() {
  ...
  `;
    ...
    // let apiQueryString = `https://api.spoonacular.com/recipes/complexSearch?query=soup&number=20&addRecipeInformation=true&sort=popularity&sortDirection=desc&apiKey=ada1ef8535a14d7695ff0ba52516335a`;
...
async function handleAdvancedFormSubmit(evt) {
    ...
    if (isPositiveNumber(preparationTimeValue)) // isPositiveNumber is ook true voor een lege string, maar een lege string hoeft niet te worden ingevuld in de api query
    {
        ...
        if (helpTextElement.innerHTML) helpTextElement.innerHTML += `<br>`;
...
async function fetchData(apiQueryString, searchSummary) {
    ...
    try {
        ...
        const response = await _axiosDefault.default.get(apiQueryString);
...
function showResults(offset1) {
    ...
    if (receivedResults.length > gallerySize) galleryElement.innerHTML += `<div class="arrow" onclick="clickLeft()" onSelectStart="return false;">&lt;</div>`;
...
//# sourceMappingURL=advancedsearch.0641b553.js.map
