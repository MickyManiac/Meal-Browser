...
async function fetchData(searchString) {
    try {
        ...
        let htmlString;
        if (searchString) {
            const result = await _axiosDefault.default.get('https://api.spoonacular.com/recipes/complexSearch?query=' + searchString + '&number=1&apiKey=ada1ef8535a14d7695ff0ba52516335a');
            if (result.data.results.length === 0) htmlString = 'Geen resultaten voor zoekterm ' + searchString + '.';
            else {
                htmlString = '<hr><div>' + result.data.results[0].id + '</div><div>' + result.data.results[0].title + '</div><div><img  alt="' + result.data.results[0].title + '" src="' + result.data.results[0].image + '"></div><hr>';
        ...
        } else document.getElementById('searchhelp').innerHTML = 'Vul een zoekterm in.';
    } catch (err) {
        console.error(err);
    }
}
...
async function fetchRecipe(recipeId) {
    ...
    try {
        let htmlString;
        const result = await _axiosDefault.default.get('https://api.spoonacular.com/recipes/' + recipeId + '/information?apiKey=ada1ef8535a14d7695ff0ba52516335a');
