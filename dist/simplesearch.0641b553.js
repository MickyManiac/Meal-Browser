...
function validCharacters(str) {
    for(let i = 0; i < str.length; i++){
        ...
        if (!(code >= 65 && code <= 90 || code >= 97 && code <= 122 || code >= 192 && code <= 255 || str.charAt(i) === ' ' || str.charAt(i) === '"' || str.charAt(i) === '&' || str.charAt(i) === '(' || str.charAt(i) === ')' || str.charAt(i) === '+' || str.charAt(i) === '-')) return false;
    }
    return true;
}
...
async function fetchData(searchString) {
    ...
    try {
        ...
        if (!receivedResults) {
            ...
            for(let i = 0; i < receivedResults.length; i++)resultsElement.innerHTML += `
...
window.fetchRecipe = async function(recipeId) {
    ...
    try {
        ...
        else if (receivedResult.sourceUrl) // Injecteer de informatie uit het zoekresultaat als html.
        selectedResultElement.innerHTML = `
