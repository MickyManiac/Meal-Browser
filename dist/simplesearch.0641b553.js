    ...
    try {
        ...
        selectedResultElement.innerHTML = `Geen verdere  informmatie gevonden voor dit recept.`;
            ...
            if (receivedResult.extendedIngredients && receivedResult.extendedIngredients.length > 0) {
                ingredients = `<ul>`;
                for(let i = 0; i < receivedResult.extendedIngredients.length; i++)ingredients += `<li>${receivedResult.extendedIngredients[i].original}</li>`;
                ingredients += `</ul>`;
            }
            selectedResultElement.innerHTML = `
                ...
                <hr>
            `;
        } else selectedResultElement.innerHTML = `Geen externe link gevonden voor dit recept.`;
    } catch (err) {
        console.error(err);
    }
};
